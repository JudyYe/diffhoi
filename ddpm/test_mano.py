...
import argparse
import cv2
from MANO.mano.utils import Mesh
from ddpm.mow import center_vertices
...
def main():
    ...
    im_path = osp.join(data_dir, 'images/{0}.jpg').format(anno['image_id'])
    obj_path = osp.join(data_dir, 'models/{0}.obj').format(anno['image_id'])
    ...
    print("Loading hand...")
    rhm_path = '/glusterfs/yufeiy2/pretrain_model/mano/MANO_RIGHT.pkl'
    rh_model = mano.load(
        model_path=rhm_path,
        model_type='mano',
        num_pca_comps=45,
    ...
    plt.imshow(im_vis)
    plt.savefig(osp.join(vis_dir, 'zhe%04d.png' % idx))
    print(model.vertices.shape, model.faces.shape)
    obj = Meshes(model.vertices, model.faces)
    hand = Meshes(hand.vertices, hand.faces)
    hoi = mesh_utils.join_scene([obj, hand]).to('cuda:0')
    image_utils.save_gif(mesh_utils.render_geom_rot(hoi, scale_geom=True), 
        osp.join(vis_dir, '%04dzhe' % idx))
