...
def update_pythonpath_relative_hydra():
    ...
    logging.info('PYTHONPATH: {}'.format(os.environ["PYTHONPATH"]))
...
@hydra.main(config_path='./configs/', config_name='sem_glide')
def hydra_main(config):
    ...
    if not config.environment.resume:
        print('dangerous!! wipe out %s' % config.exp_dir)
        os.system('rm -r %s' % config.exp_dir)
    # add slurm 
